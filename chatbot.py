...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        tril = torch.tril(torch.ones(T, T, device=x.device))  # Dynamic lower-triangular mask
        wei = wei.masked_fill(tril == 0, float('-inf'))  # Apply the mask to ignore future tokens
